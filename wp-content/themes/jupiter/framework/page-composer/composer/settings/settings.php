...
class WPBakeryVisualComposerSettings {
    ...
    public function init() {
        ...
        add_settings_section($this->option_group.'_default',
            __('General settings', 'mk_framework'),
            array(&$this, 'setting_section_callback_function'),
            $this->page);
        if( WPBakeryVisualComposer::getInstance()->isPlugin() ) {
            add_settings_field(self::$field_prefix.'content_types', __("Content types", "mk_framework"), array(&$this, 'content_types_field_callback'), $this->page, $this->option_group.'_default');
        } else {
            add_settings_field(self::$field_prefix.'theme_content_types', __("Themes content types", "mk_framework"), array(&$this, 'theme_content_types_field_callback'), $this->page, $this->option_group.'_default');
        }
        add_settings_field(self::$field_prefix.'groups_access_rules', __("Rules for users groups", "mk_framework"), array(&$this, 'groups_access_rules_callback'), $this->page, $this->option_group.'_default');
    ...
    public function groups_access_rules_callback() {
        ...
        $show_types = array(
            'all' => __('Show visual composer & default editor', 'mk_framework'),
            'only' => __('Show only visual composer', 'mk_framework'),
            'no' => __("Don't allow to use visual composer", 'mk_framework')
        );
            ...
            ?>
                ...
                <div id="wpb-settings-group-<?php echo $key ?>" class="accordion-body">
                    <div class="visibility settings-block">
                        <label for="wpb_composer_access_<?php echo $key ?>"><b><?php _e('Visual composer access', 'mk_framework') ?></b></label>
                    ...
                    <div class="shortcodes settings-block">
                        <div class="title"><b><?php echo _e('Allowed shortcodes', 'mk_framework') ?></b> </div>
    ...
    public function content_types_field_callback() {
        ...
        ?>
        <p class="description indicator-hint"><?php _e("Select for which content types Visual Composer should be available during post creation/editing.", "mk_framework"); ?></p>
        <?php
    }
...
<div class="wrap" id="wpb-js-composer-settings">
    <?php screen_icon(); ?>
    <h2><?php _e('Page Composer Settings', 'mk_framework'); ?></h2>
