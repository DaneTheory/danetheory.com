    ...
    $.wpb_composer = {
        ...
        addLastClass: function(dom_tree) {
            ...
            $dom_tree.children(".wpb_sortable").each(function (index) {
                ...
                if (cur_el.hasClass('wpb_vc_column') || cur_el.hasClass('wpb_vc_tabs') || cur_el.hasClass('wpb_vc_accordions')) {
...
function showEditForm(element) {
    ...
    jQuery('#visual_composer_edit_form').html(jQuery('#wpb_vc_loading').text()).show().css({"padding" : 30});
    ...
    // since 2.8 ajaxurl is always defined in the admin header and points to admin-ajax.php
    jQuery.post(ajaxurl, data, function(response) {
        jQuery('#visual_composer_edit_form').html(response).css({"padding" : 0});
...
function saveFormEditing(element) {
    ...
    jQuery("#visual_composer_edit_form .wpb_vc_param_value").each(function(index) {
        ...
        if ( jQuery(this).hasClass("visual_selector") ) {
            new_value = jQuery(this).val();
        }
        if ( jQuery(this).hasClass("awesome_icons") ) {
            new_value = jQuery(this).val();
        }
...
function initTinyMce($element) {
    var textfield_id = $element.attr("id"),
        ...
        $element.closest('.edit_form_line').find('.switch-tmce').click(function () {
            ...
            if(wpautop) {
                var val = window.switchEditors.wpautop(jQuery(this).closest('.edit_form_line').find("textarea.visual_composer_tinymce").val());
        ...
        jQuery('#wpb_tinymce_content-html').trigger('click');
        jQuery('#wpb_tinymce_content-tmce').trigger('click'); // Fix hidden toolbar
        wpautop = true;
}
    ...
    //jQuery(dom_tree.selector+" > .wpb_sortable").each(function(index) {
        var element = jQuery(this),
            ...
            element.children('.wpb_element_wrapper').children('.wpb_vc_param_value').each(function(index) {
                ...
                else if ( jQuery(this).hasClass("visual_selector") ) {
                    new_value = jQuery(this).val();
                }
                else if ( jQuery(this).hasClass("awesome_icons") ) {
                    new_value = jQuery(this).val();
                }
...
function wpbAccordionInitCallBack(element) {
    element.find('.wpb_accordion_holder').not('.wpb_initialized').each(function(index) {
        ...
        var $tabs,
            ...
            add_icon = jQuery(this).closest('.wpb_element_wrapper').find('.add_icon'),
        ...
        delete_btn.click( function(e) {
            e.preventDefault();
            if(jQuery(this).hasClass('mk-delete-accordion')) {
                var group_class = '.mk-accordion-group';   
            } else if(jQuery(this).hasClass('mk-delete-tabs')) {
                var group_class = '.mk-tabs-group';
            }
            ...
            if ( answer ) {
                ...
                $tabs.find('h3.ui-state-active a').closest(group_class).remove();
                //
                save_composer_html();
            }
        });
        ...
        add_btn.click( function(e) {
            ...
            var section_template = '<div class="'+group_class+'"><h3><i class=""></i><a href="#">' + tab_title + '</a></h3><div class="row-fluid wpb_column_container not-column-inherit wpb_sortable_container"></div></div>';
        ...
        add_icon.click( function() {
              var tab_icon = $tabs.find('h3.ui-state-active i').attr('class');
            tab_icon_val = tab_icon.replace('mk-', '');
            //console.log(tab_icon_val);
            
            jQuery('#mk-icon-holder-container #icon-holder').val(tab_icon_val);
            jQuery("#mk-icon-holder-container .mk-font-icons-wrapper").find('a').each(function(){
            if(jQuery(this).attr('rel')==tab_icon_val){
                jQuery(this).siblings().removeClass('current').end().addClass('current');
            }
            });
            jQuery('.mk-icon-holder-submit').click(function() {
                tab_icon_new = jQuery('.mk-font-icons-wrapper').find('#icon-holder').val();
                //console.log(tab_icon_new);
                if(tab_icon_new != '') {
                    $tabs.find('h3.ui-state-active i').attr('class', 'mk-' + tab_icon_new);
                } else {
                    $tabs.find('h3.ui-state-active i').attr('class', '');
                }
...
function wpbAccordionGenerateShortcodeCallBack(current_top_level, inner_element_count) {
    var tab_title = current_top_level.find(".mk-accordion-group:eq("+inner_element_count+") h3 a").text();
    var tab_icon = current_top_level.find(".mk-accordion-group:eq("+inner_element_count+") h3 i").attr('class');
...
function wpbTabsGenerateShortcodeCallBack(current_top_level, inner_element_count) {
    var tab_title = current_top_level.find(".mk-tabs-group:eq("+inner_element_count+") h3 a").text();
    var tab_icon = current_top_level.find(".mk-tabs-group:eq("+inner_element_count+") h3 i").attr('class');
    if(tab_icon == undefined) {tab_icon= ''}
    output = '[vc_tab icon="'+tab_icon+'" title="'+tab_title+'"] %inner_shortcodes [/vc_tab]';
    return output;
}
