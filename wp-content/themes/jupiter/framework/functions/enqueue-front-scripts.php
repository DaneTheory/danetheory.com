...
function theme_enqueue_scripts() {
	if(!is_admin()){
		...
		if ( $mk_option['disable_smoothscroll'] == 'true') {
			wp_enqueue_script('jquery-smoothscroll', THEME_JS. '/jquery.smoothscroll.js', array('jquery'), false, $move_bottom);
		}
		...
		if ( is_rtl() ) {
		  wp_enqueue_style(  'style-rtl', THEME_DIR_URI.'/rtl.css', false, false, 'all');
		}
...
function mk_enqueue_styles() {
	...
	$body_bg .= get_post_meta( $post->ID, 'body_position', true ) ? 'background-position:'.get_post_meta( $post->ID, 'body_position', true ).' !important;' : '';
	...
	$header_bg .= get_post_meta( $post->ID, 'header_position', true ) ? 'background-position:'.get_post_meta( $post->ID, 'header_position', true ).' !important;' : '';
	...
	$banner_bg .= get_post_meta( $post->ID, 'banner_position', true ) ? 'background-position:'.get_post_meta( $post->ID, 'banner_position', true ).' !important;' : '';
	...
	$footer_bg .= get_post_meta( $post->ID, 'footer_position', true ) ? 'background-position:'.get_post_meta( $post->ID, 'footer_position', true ).' !important;' : '';
